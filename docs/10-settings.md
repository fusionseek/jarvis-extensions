...
| `multiselect` | 一排可多选的 chip（借 `capabilityChip` 那一族：选中青底青字） | 每点一下写一次；至少留一项时才可取消最后一项 |
| `secret` | 遮罩输入框 + 右端「已设置」/「未设置」徽标 + 「清除」link | 值进 Keychain（`<extension id>/<key>`），不进 SQLite、不进日志；设置页从不回显明文 |
| `directory` | 「选择…」`SettingsChipButton` + 缩写路径 + 「重置」link | 走 `NSOpenPanel`，只能选目录；路径存为字符串 |

### 命令与快捷键段

有 `commands` 的扩展，面板里在授权段之后多一段「命令」：每条命令一行，标题 14 semibold + 说明 12，
右端是快捷键键帽（`ScreenshotSettingsView` 那一枚）或「未设置」。点键帽进入**录制态**：键帽变成
「按下新的组合键…」，按下即写入、Esc 取消、⌫ 清除；与内置截图那两组键或其它扩展撞键时当场拒绝并说明。
这枚录制控件是宿主的新组件——截图区段那句"这一版还不能在这里改键"的遗留也由它一并了结。
