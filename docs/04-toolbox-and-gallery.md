...
### 扩展商店（期 A.2，设计已画，实现待期 A.2 feature）

扩展库长成商店的那一版画在 Figma 新板 `jarvis-extension-store`（`1001:2`）：搜索框（与工具箱搜索同一条策略）、
发现 / 已安装 / 可更新三档、卡片上的「新」「有更新」徽标与高风险个数、详情里的「装上之后长什么样」预览行与命令落点徽标、
能力按档位分组、来源三行（产物 / 兼容 / 更新）、更新时"多要了哪几项能力"的差异行、安装三步（取产物 → 校验 SHA-256 → 写入）、
拒装与不兼容各一屏，以及「来源与快照」那一屏（仓库、commit、本机缓存目录、ETag、检查周期、装到哪）。
header 标题改叫「扩展商店」。节点表与规则在 `jarvis-mac/specs/20260912-195035-the-store-opens/design.md`。
