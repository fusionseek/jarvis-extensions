...
| 二 | 点进去之后的功能、页面之外的入口（命令与快捷键），以及它要用到宿主的哪些能力 | `main` + `commands[]` + `capabilities[]` | 面板里那一屏、全局快捷键；第一次进入时的**统一授权弹窗** |
...
| **JS 逻辑 + 声明式原生 UI（选定）** | 代价是组件目录是**封闭**的：扩展只能用宿主提供的 31 种节点。这正是「风格统一、交互一致」这条要求的代价，而不是缺陷 |

Raycast 的扩展走的就是这条路（TypeScript 逻辑 + 原生渲染的组件目录 + 一个 GitHub 仓库当商店），
它证明了这条路撑得起一个真实的生态。入口模型也照它：**命令 + 页面**——工具箱那一行打开页面，
快捷键、页面按钮、快捷环、Inbox 卡片触发命令；快捷键由宿主按 manifest 在启动时注册，
按下才按需加载扩展的 JS。

**性能上 JS 不在渲染路径的每一帧里**：它只跑事件、能力调用与整棵树的提交；悬停光、滚动、动画、
文本编辑全在宿主，输入框的文本由宿主持有、不经 JS 往返（中文输入法的组合态因此不会被打断）。
一次交互多出来的只有两次线程切换与两次 JSON 序列化，远在一帧之内。
